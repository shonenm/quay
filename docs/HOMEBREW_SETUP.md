...
Quay uses a custom Homebrew tap for macOS distribution. The tap is automatically updated by the `publish-homebrew` job in `.github/workflows/release.yml` when a new release is published.
...
The formula is generated by the `publish-homebrew` job in `.github/workflows/release.yml`. Here's the expected structure:
